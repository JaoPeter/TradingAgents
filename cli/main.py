...
def get_user_selections():
    ...
    # Step 2: Trading style and timeframe
    console.print(
        create_question_box(
            "Step 2: Trading Style & Timeframe",
            "Select how you intend to trade this instrument",
        )
    )
    tf_selections = select_trading_style_and_tf()
    ...
    return {
        ...
        "trading_style": tf_selections["trading_style"],
        "primary_tf": tf_selections["primary_tf"],
        "confirm_tf": tf_selections["confirm_tf"],
...
def run_analysis():
    ...
    config["primary_tf"] = selections.get("primary_tf", "1d")
    config["confirm_tf"] = selections.get("confirm_tf", "")
    ...
    with Live(layout, refresh_per_second=4) as live:
        ...
        init_agent_state = graph.propagator.create_initial_state(
            ...
            trading_style=selections.get("trading_style", "swing"),
            primary_tf=selections.get("primary_tf", "1d"),
            confirm_tf=selections.get("confirm_tf", ""),
        )
        # Pass callbacks to graph config for tool execution tracking
        # (LLM tracking is handled separately via LLM constructor)
        args = graph.propagator.get_graph_args(callbacks=[stats_handler])

        # Stream the analysis with robust error handling
        trace = []
        try:
            ...
            log_api_error(
                error_log_path=results_dir / "errors.jsonl",
                source="cli.run_analysis.stream",
                error=exc,
                context={
                    "ticker": selections.get("ticker"),
                    "analysis_date": selections.get("analysis_date"),
                    "trading_style": selections.get("trading_style"),
                    "primary_tf": selections.get("primary_tf"),
                    "confirm_tf": selections.get("confirm_tf"),
                },
            )
            raise
        ...
        if not trace:
            raise RuntimeError("No output received from graph stream.")
